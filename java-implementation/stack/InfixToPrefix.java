...
* InfixToPrefix
*
* The InfixToPrefix takes the infix expression from command line (first argument)
* and converts it to prefix expression and prints the output
*
...
public class InfixToPrefix {
	...
	*/
	...
	public static void main(String[] args) throws Exception {
		if(args.length==0) {
			...
			System.out.println("prefix expression - "+itp.convert(args[0]));
		}
	}
	...
	* @return String prefix expression string
	* @throws Exception if there is invalid data in stack
	*/		
	public String convert(String infix) throws Exception {
		String prefix=new String(), reverse=null, postfix=null;
		
		reverse=new StringBuilder(infix).reverse().toString();
		reverse=replaceBrackets(reverse);
		
		InfixToPostfix itp=new InfixToPostfix();
		postfix=itp.convert(reverse);
		//System.out.println("postfix -- "+postfix);
		
		prefix=new StringBuilder(postfix).reverse().toString();
		
		return prefix;
	}

	/** 
	* replaces all the open brackets with closed brackets and vice versa in te expression
	* @param expression input string in which the brackets have to be replaced
	* @return String expression with the brackets replaced
	*/
